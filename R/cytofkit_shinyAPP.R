...
#' @export
#' @examples 
#' if (interactive()) cytofkit::cytofkitShinyAPP2()
cytofkitShinyAPP2 <- function(obj) {
    ...
    shinyApp(
        ...
        server = function(input, output, session) {
            ...
            v <- reactiveValues(data = analysis_results, sampleInfo = sampleInformation)
            c <- reactiveValues(clusterCol = list())
            p <- reactiveValues(progressionCluster = progCluster)

...
performed directly on the high dimensional data. Check the function `cytof_cluster` for 
...
able to compare the overall relatedness of cells and embed the cells taking into account the nonlinear geodesic distances between cells. By plotting the marker expression along the *ISOMAP* dimension 1 or dimension 2 with lowess regression, markers with expression pattern that highly correlated with progression can be defined as regulatory markers. Check the function `cytof_progression` for implementation of this step.
...
Among the saved results, an R data object will be saved with suffix of *.RData*, this is specially for loading to the shiny web APP for interactive visualization and exploration of the analysis results. The shiny APP can be launched with command `cytofkitShinyAPP()` or through this link 
